...
public class OAuthClient {
	...
	public OAuth requestToken(OAuth oauth) throws Exception {
		...
		String queryString = getOauthParams(url, "GET", oauth
				.getOauth_consumer_secret(), "", oauth.getParams());

		String responseData = http.get(url, queryString);
	...
	public Account getAccount(OAuth oauth) throws Exception {
		...
		Document doc = dbd.parse(new ByteArrayInputStream(myInfo
				.getBytes("UTF-8")));
		XPathFactory f = XPathFactory.newInstance();
		XPath path = f.newXPath();

		int ret = Integer.parseInt(path.evaluate("root/ret", doc));
		if (ret != 0) {
			return account;
		}

		String name = path.evaluate("root/data/name", doc);
		String nick = path.evaluate("root/data/nick", doc);
		String head = path.evaluate("root/data/head", doc);
		String isvip = path.evaluate("root/data/isvip", doc);
		String sex = path.evaluate("root/data/sex", doc);
		String fansnum = path.evaluate("root/data/fansnum", doc);
		String idolnum = path.evaluate("root/data/idolnum", doc);
		String tweetnum = path.evaluate("root/data/tweetnum", doc);
